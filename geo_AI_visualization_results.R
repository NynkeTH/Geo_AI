# Visualization of the results of the SGD classifier
# input from geo_AI_stripped with export.py
...
#import data
pythondata <- read_csv("output_python_geoAI.csv")

#make vector of GEF names
GEFs = unique(pythondata$GEFname)

#make plot for each GEF
for (GEF in GEFs){
  #filter data for 1 GEF
  van1GEF <- pythondata %>% 
    filter(GEFname == GEF) %>% 
    select(soilname, pred_soilname, depth) %>% 
    rename(prediction = pred_soilname, real = soilname) %>% 
    gather("real_or_pred", "soil", c(real, prediction)) %>% 
    mutate(laag = cut(depth, 10, labels=rev(c(1:10)))) %>% 
    mutate(laag = factor(laag, levels=c(1:10))) %>% 
    filter(!is.na(soil))
  
  #make plot for GEF
  ggplot(van1GEF, aes(real_or_pred, depth, color = soil)) +
    geom_point(shape = 15) +
    facet_wrap(~ laag, scales = 'free_y', nrow = 1) +
    ggtitle(GEF) +
    ...
    ggsave(paste0("viz_", GEF, ".jpg"), dpi = 300, height = 4, width = 8)
}
...
pythondata %>% 
  ...
  scale_color_manual(values = c("#000080", "#002aff", "#00d4ff", "#7bff7b", "#ffe600", "#ff4800", "#800000")) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.3), axis.title.x = element_blank(), plot.title = element_text(hjust = 0.5)) +
  ggsave("all_GEFs.jpg", dpi = 300, height = 8, width = 10)
...
ggplot(cm, aes(pred_soilname, soilname)) + 
  ...
  xlab("predicted soil") +
  ylab("real soil") +
  ggsave("cm_numbers.jpg", height = 4, width = 5)
...
ggplot(cm, aes(pred_soilname, soilname)) + 
  ...
  xlab("predicted soil") +
  ylab("real soil") +
  ggsave("cm_ratio.jpg", height = 4, width = 5)
