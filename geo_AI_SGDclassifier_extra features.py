...
df_noise_test['sgdclassifier']= math.nan
df_noise_train['sgdclassifier']= math.nan 

# train SGD classifier with train set
clf = SGDClassifier(loss="hinge", penalty="l2", max_iter=1000, tol=1e-4, class_weight= 'balanced')
df_train['sgdclassifier'] = clf.fit(X_train,y_train).predict(X_train)
print("Accuracy SGDClassifier train set:", accuracy_score(y_train, df_train['sgdclassifier']))

# apply trained model to test set
df_test['sgdclassifier'] = clf.predict(X_test)
print("Accuracy SGDClassifier test set:", accuracy_score(y_test, df_test['sgdclassifier']))

# to df and add labels for clarity and plotting
coefs = pd.DataFrame(clf.coef_, columns = ['feature_coef_1', 'feature_coef_2', 
                                           'feature_coef_3', 'feature_coef_4', 
                                           'feature_coef_5', 'feature_coef_6', 
                                           'feature_coef_7'])
coefs['soilnames'] = soillabels
coefs = pd.wide_to_long(coefs, stubnames = 'feature_coef_', i='soilnames', j = 'feature')
coefs = coefs.reset_index()
coefs['feature_name'] = np.select(condlist = [(coefs['feature'] == 1), (coefs['feature'] == 2), 
     (coefs['feature'] == 3), (coefs['feature'] == 4), 
     (coefs['feature'] == 5), (coefs['feature'] == 6), 
     (coefs['feature'] == 7)], 
     choicelist = ['qc', 'fs', 'sv', 'qc_average_before', 'qc_average_after', 'fs_average_before', 'fs_average_after'], default='empty')

# plot the coefficients
import seaborn as sns
sns.set(style='whitegrid')
_ = sns.barplot(x='soilnames', y = 'feature_coef_', hue = 'feature_name', data = coefs, palette = 'tab10')
_ = plt.legend()
_ = plt.tight_layout()
_ = plt.ylabel('feature coefficient')
_ = plt.xlabel('soil class')
plt.savefig("feature_coefficients_SGDclassifier_extra features.jpg", dpi = 300)

# translate class back to soilname
df_train['pred_soilname'] = le.inverse_transform(df_train['sgdclassifier'])
df_test['pred_soilname'] = le.inverse_transform(df_test['sgdclassifier'])
...
df_traintestnoise.to_csv("output_python_geoAI_extra features.csv", index = False)
