...
# input from geo_AI_SGDclassifier_extra features.py
...
#import data
pythondata <- read_csv("output_python_geoAI_extra features.csv")
...
pythondata %>% 
  ...
  ggsave("all_GEFs2.jpg", dpi = 300, height = 8, width = 10)
...
ggplot(cm, aes(pred_soilname, soilname)) + 
  ...
  xlab("predicted soil") +
  ylab("real soil") +
  ggsave("cm_numbers2.jpg", height = 4, width = 5)
...
ggplot(cm, aes(pred_soilname, soilname)) + 
  ...
  xlab("predicted soil") +
  ylab("real soil") +
  ggsave("cm_ratio2.jpg", height = 4, width = 5)
